                ...
                Column(
                        ...
                        Button( onClick = {
                            val day=viewmodel.getday(viewmodel.date){day->
                                if (day!= null) {
                                    ...
                                    viewmodel.newday(Day(id=0,date=date,steps=0))
                                    val day=viewmodel.getday(viewmodel.date) { day ->
                                        if (day!= null) {
                                            val newtask=Task(name = viewmodel.name, done = false, dayId=day.id,id=0)
                                            viewmodel.newtask(newtask)
                    ...
                    Row(
                        ...
                        Button( onClick = {
                            ...
                            val daysRemainingList = mutableListOf<String>()
                            for (day in currentDayOfMonth..totalDaysInMonth) {
                                val date = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).apply {
                                    calendar.set(Calendar.DAY_OF_MONTH, day)
                                }.format(calendar.time)
                                daysRemainingList.add(date)
                            }
                            daysRemainingList.size
                            daysRemainingList[1]


                            viewmodel.viewModelScope.launch {
                                for (i in 0 until daysRemainingList.size) {
                                    val day=viewmodel.getday(daysRemainingList[i]){day->
                                        if (day!= null) {
                                            val newtask=Task(name = viewmodel.name, done = false, dayId=day.id,id=0)
                                            viewmodel.newtask(newtask)
                                            //viewmodel.name=""
                                            ...
                                            val date= viewmodel.dateConverter.toDateFromString(daysRemainingList[i])
                                            viewmodel.newday(Day(id=0,date=date,steps=0))
